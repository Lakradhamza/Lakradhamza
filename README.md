# Hi there, I'm Hamza LAKRAD 👋

**Business Intelligence Consultant** specializing in data transformation, ETL processes, and advanced analytics solutions.

[![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)](https://powerbi.microsoft.com/)
[![Talend](https://img.shields.io/badge/Talend-FF6D70?style=for-the-badge&logo=talend&logoColor=white)](https://www.talend.com/)
[![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)](https://www.mysql.com/)
[![Oracle](https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white)](https://www.oracle.com/)
[![Qlik](https://img.shields.io/badge/Qlik-009848?style=for-the-badge&logo=qlik&logoColor=white)](https://www.qlik.com/)
[![Tableau](https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white)](https://www.tableau.com/)

---

## 👨‍💻 About Me

I'm a Business Intelligence professional passionate about transforming raw data into actionable insights. With expertise in ETL architecture, data modeling, and visualization, I help organizations make data-driven decisions and optimize their business processes.

**Core Competencies:**
- 📊 Data Visualization & Dashboard Development
- 🔄 ETL Process Design & Automation
- 🗄️ Data Modeling (Star Schema, Snowflake)
- ✅ Data Quality Management & Governance
- 📈 Business Intelligence & Analytics
- 🎯 KPI Development & Performance Tracking

---

## 💼 Professional Experience

### 🏦 BI Consultant - Tax Declaration Automation (SIMPL)
**Mercure IT | Client: SAHAM Bank / DGI Morocco**  
*November 2025 - December 2025*

- Redesigned ETL workflows in **Talend** to accommodate dynamic Excel file structures
- Implemented automated compliance validation and anomaly detection with email alerting
- Generated XML files compliant with Moroccan tax authority (DGI) specifications
- Ensured data accuracy and regulatory compliance for tax reporting
- Reduced manual processing time by 70% through automation

### 💳 BI Consultant - Collection Activity Management
**Mercure IT | Client: EQDOM Bank**  
*October 2025 - November 2025*

- Developed comprehensive **Power BI dashboards** for financial monitoring and collection tracking
- Integrated data from **Oracle** databases with optimized queries
- Implemented **Row-Level Security (RLS)** with **LDAP authentication** for secure data access
- Utilized **Power Query (M)** for complex data transformations and semantic modeling
- Created dynamic reports enabling real-time decision-making for collection strategies

### ✈️ BI Consultant - Subsidiary Financial Performance Tracking
**Royal Air Maroc**  
*October 2022 - August 2024*

- Designed and deployed decision-support solutions for multi-subsidiary financial analysis
- Automated ETL processes to streamline data integration from multiple sources
- Established data quality controls and validation rules to ensure reporting accuracy
- Created executive dashboards for strategic financial oversight
- Improved reporting efficiency by 60% through process optimization

---

## 🚀 Featured Projects

### 📊 Financial KPI Dashboard
Advanced Power BI solution featuring comprehensive financial metrics including margin analysis, EBITDA tracking, and time series forecasting with Star Schema data modeling.

**Technologies:** Power BI, DAX, SQL, Star Schema  
**Key Features:**
- Real-time financial KPI tracking
- Time series analysis with trend forecasting
- Interactive drill-down capabilities
- Custom DAX measures for complex calculations

### ✈️ Airline Analytics Dashboard
Interactive Qlik Sense visualization platform for analyzing flight operations, passenger traffic patterns, and route performance metrics.

**Technologies:** Qlik Sense, SQL, Data Modeling  
**Key Features:**
- Flight operations analysis
- Passenger traffic visualization
- Route performance monitoring
- Operational efficiency metrics

### 🛒 E-commerce Analytics Pipeline
End-to-end data integration solution using Talend for ETL operations and Qlik Sense for sales trend analysis and business intelligence.

**Technologies:** Talend, Qlik Sense, SQL  
**Key Features:**
- Automated data extraction from multiple sources
- Sales trend analysis and forecasting
- Customer behavior analytics
- Product performance tracking

---

## 🎓 Education

**Master's Degree in Big Data & Data Science**  
*2022 - 2024*  
Specialized in Business Intelligence, Big Data Technologies, and Advanced Data Processing

**Bachelor's Degree in Mathematics & Computer Science**  
*2018 - 2022*  
Faculty of Sciences Ben M'sik, Casablanca

---

## 🛠️ Technical Stack

### Business Intelligence & Analytics
```
📊 Data Visualization: Power BI, Qlik Sense, Tableau
🔄 ETL Tools: Talend, SSIS
🗄️ Databases: Oracle, MySQL, SQL Server, PostgreSQL
📐 Data Modeling: Star Schema, Snowflake, Data Vault
💻 Languages: SQL, DAX, M (Power Query), Python (Basic)
```

### Core Skills
- **Data Warehousing:** Dimensional Modeling, ETL Design
- **Data Analysis:** Statistical Analysis, Trend Analysis, KPI Development
- **Data Quality:** Data Profiling, Cleansing, Validation
- **Reporting:** Interactive Dashboards, Automated Reports, Ad-hoc Analysis
- **Version Control:** Git, GitHub
- **Methodologies:** Agile, Scrum

---

## 🌐 Languages

- **Arabic:** Native Speaker
- **French:** Professional Proficiency (C1)
- **English:** Professional Proficiency (B2)

---

## 📊 GitHub Stats

<div align="center">
  
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=hamzalakrad&show_icons=true&theme=tokyonight&hide_border=true&count_private=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=hamzalakrad&layout=compact&theme=tokyonight&hide_border=true)

</div>

---

## 📈 Activity Graph

[![Hamza's github activity graph](https://github-readme-activity-graph.vercel.app/graph?username=hamzalakrad&theme=tokyo-night)](https://github.com/hamzalakrad)

---

## 🏆 Certifications & Learning

- 🎯 Business Intelligence Development
- 📊 Advanced Power BI & DAX
- 🔄 ETL Best Practices
- 🗄️ Data Modeling & Architecture
- ☁️ Cloud Data Solutions

---

## 📫 Let's Connect

I'm always open to discussing Business Intelligence, data analytics, or potential collaboration opportunities.

<div align="center">

[![Email](https://img.shields.io/badge/Email-hamzalakrad42%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:hamzalakrad42@gmail.com)
[![Phone](https://img.shields.io/badge/Phone-%2B212%20625%20562%20438-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)](tel:+212625562438)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/hamza-lakrad-222214232/)
[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-FF5722?style=for-the-badge&logo=google-chrome&logoColor=white)](https://hamzalakrad.github.io)

</div>

---

<div align="center">

### 💡 "Turning Data into Decisions, One Dashboard at a Time"

![Profile Views](https://komarev.com/ghpvc/?username=hamzalakrad&color=blueviolet&style=for-the-badge)

⭐️ From [Hamza LAKRAD](https://github.com/hamzalakrad) | Feel free to explore my repositories and reach out for collaboration!

</div>

---

## 📝 Latest Blog Posts
<!-- BLOG-POST-LIST:START -->
<!-- BLOG-POST-LIST:END -->

---

## 🤝 Open to Opportunities

I'm currently open to:
- 💼 BI Consultant positions
- 🚀 Freelance projects
- 👥 Collaboration on data analytics initiatives
- 📚 Knowledge sharing and mentoring

**Let's build something amazing together!** 🚀